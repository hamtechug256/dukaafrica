          ...
          <div className="flex-1 min-w-0">
            {posts.length > 0 ? (
              <>
                ...
                {totalPages > 1 && (
                  <nav className="flex items-center justify-center gap-2 mt-12" aria-label="Pagination">
                    {page > 1 && (
                      <Link href={`/blog/category/${slug}?page=${page - 1}`} className="px-5 py-2.5 rounded-xl text-sm font-medium border border-[oklch(0.92_0.01_85)] dark:border-[oklch(0.25_0.02_45)] hover:bg-white dark:hover:bg-[oklch(0.15_0.02_45)] transition-colors">Previous</Link>
                    )}
                    <span className="px-4 py-2 text-sm text-gray-500">Page {page} of {totalPages}</span>
                    {page < totalPages && (
                      <Link href={`/blog/category/${slug}?page=${page + 1}`} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white transition-colors" style={{ background: 'linear-gradient(135deg, oklch(0.6 0.2 35), oklch(0.55 0.18 40))' }}>Next</Link>
                    )}
                  </nav>
                )}
              </>
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <h3 className="text-lg font-bold mb-2 text-gray-900 dark:text-gray-100">No articles in this category</h3>
