...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (store.payoutMethod === 'MOBILE_MONEY') {
        accountBank = MOBILE_MONEY_BANKS[store.country] || 'MTNUG'
        accountNumber = store.payoutPhone?.replace(/\D/g, '') || ''
        narration = `DuukaAfrica payout to ${store.payoutPhone}`
      } else {
        // Bank transfer - use bank name as bank code (would need proper mapping in production)
        accountBank = store.payoutBankName || ''
